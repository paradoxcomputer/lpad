#!/usr/bin/env bash
# Real-proof (RISC0_DEV_MODE unset) verification of the private paths on rc4.
...
ENVF=scripts/bootstrap.rc4-real.env
...
if ! { [ -f "$ENVF" ] && grep -q LPAD_LBP_POOL_ID "$ENVF"; }; then
  say "BOOTSTRAP DID NOT COMPLETE - last lines:"; tail -25 /tmp/lpad-rc4-real-bootstrap.log | tee -a "$LOG"; exit 1
fi
